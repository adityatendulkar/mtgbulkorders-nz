...
# Pacing. Cloudflare error 1015 (zone-owner rate limit) kicks in around 10 requests/sec from a
# single IP on api.mtgsingles.co.nz and replies with Retry-After (typically 10s). Keep our
# per-card cadence well under that and back off generously when we do trip it.
INTER_CARD_DELAY_SEC = 1.0
RATE_LIMIT_FALLBACK_SLEEP_SEC = 11.0
MAX_ATTEMPTS = 5
...
def _build_log_entry(card, url, attempt, started_at, response=None, result=None, error=None, found=None, retry_after=None):
...
def _parse_retry_after(value):
    """Parse a Retry-After header (seconds) into a float, falling back to None."""
    if not value:
        return None
    try:
        return float(value)
...
def scrape_prices(cards, vendors, optional_cards=None, progress_callback=None, request_log_callback=None):
    ...
    for idx, card in enumerate(unique_cards_ordered, 1):
        ...
        # Fresh impersonating session per card — chrome120 fingerprint slips past Cloudflare's
        # JA3/TLS check that blocks default Python `requests` from datacenter hosts. The session
        # is reused across retries within this card so the connection pool stays usable.
        ...
        try:
            for attempt in range(MAX_ATTEMPTS):
                ...
                log_entry = None
                sleep_after = 0.0  # honored in finally so the log row goes out before we wait
                try:
                    r = session.get(url, headers=HEADERS, params=params, timeout=15)

                    if r.status_code == 429:
                        retry_after = _parse_retry_after(r.headers.get("Retry-After")) or RATE_LIMIT_FALLBACK_SLEEP_SEC
                        log_entry = _build_log_entry(card, url, attempt + 1, started_at, response=r, result="rate-limited", retry_after=retry_after)
                        if attempt < MAX_ATTEMPTS - 1:
                            print(f" - Rate-limited (waiting {retry_after:.0f}s)", end="")
                            sleep_after = retry_after
                            continue
                        print(f" - Rate-limited (gave up after {MAX_ATTEMPTS} attempts)")
                        break
                    ...
                    if attempt < MAX_ATTEMPTS - 1:
                        ...
                        sleep_after = 0.5 + attempt * 0.5  # small increasing backoff
                    else:
        ...
        # Delay between cards — keeps us under the per-IP rate-limit refill rate
        if idx < total_unique:
            time.sleep(INTER_CARD_DELAY_SEC)
